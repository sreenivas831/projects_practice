...
DBConnection("jdbc:mysql://localhost:3306/ecommerce", "root", "Srinu143#");
...
while (rst.next()) {
 out.println(rst.getInt("ID") + ": " +rst.getString("color") + " "+ rst.getString("name") + " costs: $" +rst.getDouble("price") + "<Br>");
 }
 }
...
if(inTable.next())
 out.println(inTable.getInt("ID") + ": " +inTable.getString("color") + " "+ inTable.getString("name") + " costs: $" +inTable.getDouble("price") + "<Br>");
